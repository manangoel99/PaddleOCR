...
|      use_wandb  |    设置是否启用W&B进行可视化log展示 |          False        |    [教程地址](https://docs.wandb.ai/) |
...
| :---------------------: |  :---------------------:   | :--------------:  |   :--------------------:   |
|          project              |     本次运行的项目 | - | \
|          name                 |     本次运行的名称/别名 | 由wandb随机生成 | \ 
|          id                   |     本次运行的ID    | 由wandb随机生成     | \
|          entity               |     本次运行的用户或团队         | 登陆的用户 | \
|          save_dir             |     模型和其他数据保存的本地目录 | wandb | \
|          config               |     模型的配置项 | None | \
